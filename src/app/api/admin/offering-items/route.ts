...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await queryAll(
      `SELECT * FROM OfferingSupply ${whereSql} ORDER BY sortOrder ASC, id DESC LIMIT ${pageSize} OFFSET ${offset}`,
      ...params
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    await auditLog({
      ...
      details: { target: 'offering_item', name },
      status: 'success',
    });
...
export async function PUT(req: NextRequest) {
  try {
    ...
    await auditLog({
      userId: session?.sub,
      action: 'admin_update_config',
      details: { target: 'offering_item', id },
      status: 'success',
    });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await auditLog({
      userId: session?.sub,
      action: 'admin_update_config',
      details: { target: 'offering_item', ids, count: ids.length },
      status: 'success',
    });
