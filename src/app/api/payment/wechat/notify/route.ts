...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await paymentService.handleCallback('wechat', rawBody, headers);
    ...
    if (!order) {
      console.error('微信回调订单不存在:', result.orderNo);
    ...
    if (isNaN(paidAmount) || Math.abs(order.amount - paidAmount) > 0.01) {
      console.error('微信回调金额不匹配:', order.amount, paidAmount);
    ...
    console.error('微信支付回调处理失败:', error?.message);
