...
export default function AgentRecordsPage() {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/agent/records?${params}`);
      if (res.ok) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h2 className="text-lg font-bold text-gray-900">客户排盘记录</h2>
          <p className="text-sm text-gray-500">名下客户使用四大命理排盘/解读的记录，共 {total} 条</p>
        </div>
              ...
              records.map((r) => (
                <tr key={r.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded ${TYPE_COLOR[r.type] || 'bg-gray-100 text-gray-600'}`}>
                      {r.type}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-gray-900">{r.userName || '未命名用户'}</div>
                    <div className="text-xs text-gray-400">{r.userEmail || r.userPhone || r.userId}</div>
                  </td>
                  <td className="px-4 py-3 text-xs text-gray-600">
                    {r.type === '八字' || r.type === '紫微' ? (
                      <span>
                        {r.name && <span className="text-gray-900">{r.name} · </span>}
                        {GENDER_LABEL[r.gender] && <span>{GENDER_LABEL[r.gender]} · </span>}
                        {r.birthDate}
                        {r.birthTime && r.birthTime !== '未知' ? ` ${r.birthTime}` : ''}
                      </span>
                    ) : r.type === '奇门' ? (
                      <span>起局时间：{r.birthDate}</span>
                    ) : (
                      <span>起卦：{r.birthTime || '未知'} · {String(r.birthDate || '').slice(0, 60)}</span>
                    )}
                  </td>
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 text-sm border rounded-lg disabled:opacity-50 bg-white"
          >
            上一页
          </button>
          <button
